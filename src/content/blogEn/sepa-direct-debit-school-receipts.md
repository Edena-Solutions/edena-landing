...
faqs:
    - question: "What is a SEPA batch and why do schools use it?"
      answer: "A SEPA batch is a file that groups several direct debit orders to send them to the bank at once. Schools and nurseries use it to charge monthly fees to hundreds of families without processing receipt by receipt. Instead of loose charges, a single submission is generated with all the debits for the period."
    - question: "What's the difference between CORE and B2B in SEPA?"
      answer: "The CORE scheme is the usual one for debits to individuals (families) and allows the debtor to return the payment within a wide window. The B2B scheme is for operations between companies, with different return periods. For charging fees to families, CORE is the norm; always confirm with your bank and accountant which scheme applies to each case."
    - question: "Why are direct debits returned?"
      answer: "The most common causes are: incorrect or outdated IBAN, insufficient funds, unsigned or expired mandate, and account-holder data that doesn't match. A significant portion of returns are not real non-payments, but data errors that a system with digital mandates and IBAN validation prevents before generating the batch."
    - question: "Is it mandatory to have the mandate signed by the family?"
      answer: "Yes. To direct debit a receipt you need a valid SEPA mandate signed by the account holder, with the required information and kept so you can prove it if there is a return for a non-existent mandate. Managing mandates on loose paper is the main cause of problems in a returns audit."
    - question: "Does Edena generate SEPA batches automatically?"
      answer: "Edena's billing module manages recurring billing and family receipts within the platform, with arrears analysis. The exact scope of file generation and reconciliation depends on the contracted module and the configuration with your bank; ask in the demo to confirm which circuit is covered without manual work."
---

<strong>SEPA Direct Debit in Schools: Error-Free Receipt Collection</strong>
...
<strong>What a SEPA batch is and how it fits in a center</strong>
...
<strong>The SEPA mandate: the document that holds it all together</strong>
...
<strong>CORE vs B2B: use the right scheme</strong>
...
<strong>Why receipts are returned (and how many are avoidable)</strong>
...
<strong>The hidden cost of returns</strong>
...
<strong>How to automate direct debit properly</strong>
...
<strong>Edena and recurring fee billing</strong>
...
<strong>Context in Spain: fees, arrears, and treasury</strong>
